...
// UnmarshalText unmarshal an instance of ArgBig into an array of bytes
...
// UnmarshalText unmarshal from text
func (a *ArgAddress) UnmarshalText(input []byte) error {
	if !hex.IsValid(string(input)) {
		return fmt.Errorf("invalid address, it needs to be a hexadecimal value")
	}

	str := strings.TrimPrefix(string(input), "0x")
	*a = ArgAddress(common.HexToAddress(str))
	return nil
}

// Address returns an instance of common.Address
func (a *ArgAddress) Address() common.Address {
	result := common.Address{}
	if a != nil {
		result = common.Address(*a)
	}
	return result
}
